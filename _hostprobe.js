...
(async()=>{
  const tok=await token("https://www.googleapis.com/auth/cloud-platform");
  if(!tok){console.log("토큰 실패(cloud-platform scope)");return;}
  const r=await httpReq({host:"firebasehosting.googleapis.com",path:"/v1beta1/sites/hoojecalendar",method:"GET",headers:{"Authorization":"Bearer "+tok}});
  console.log("Hosting 사이트 조회:",r.status);
